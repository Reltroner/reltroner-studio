...
import { processVideoDirectives, injectVideoEmbeds } from "@/lib/utils/videoEmbeds";
...
export async function getCollectionEntry(collection, slug) {
  ...
  try {
    const { transformedMarkdown, placeholders } = processVideoDirectives(parsedFile.content);
    const processed = await remark().use(gfm).use(html, { sanitize: false }).process(transformedMarkdown);
    const { contentHtml, headings } = decorateContentHtml(processed.toString());
    const finalContentHtml = injectVideoEmbeds(contentHtml, placeholders);
...
export async function getCommandPaletteEntries() {
  ...
  const { ECOSYSTEM_APPS } = require('@/lib/constants/apps');
  const ecosystemEntries = ECOSYSTEM_APPS.map((app) => ({
    name: app.name,
    path: app.url,
    group: 'Ecosystem Apps',
    description: app.description,
  }));

  return [...ecosystemEntries, ...STATIC_ROUTE_ITEMS, ...items.flat()];
}